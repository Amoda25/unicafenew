    ...
    const renderDashboardOverview = () => {
        ...
        return (
            <div style={{ display: 'flex', flexDirection: 'column', gap: '32px' }}>
                ...
                {(() => {
                    // Aggregate all item sales from all orders
                    const itemSalesMap = {};
                    orders.forEach(order => {
                        order.items?.forEach(item => {
                            const name = item.name?.en || item.name;
                            ...
                            if (!itemSalesMap[name]) {
                                itemSalesMap[name] = { qty: 0, price: item.price || 0, category: item.category || 'Meals' };
                            }
                    ...
                    const threshold = avgQty * 0.3; // below 30% of average = low demand

                    const topSeller = allItems.sort((a, b) => b[1].qty - a[1].qty)[0]?.[0] || 'our bestseller';

                    // Determine promotion type per item
                    ...
                    const lowDemandItems = allItems
                        ...
                        .map(([name, data], idx) => {
                            ...
                            if (promoType === 'flash') {
                                const discPct = dropPct > 70 ? 20 : 10;
                                suggestion = `Run a ${discPct}% Flash Sale to spike demand fast!`;
                                discountLabel = `${discPct}% OFF`;
                                promoColor = '#ef4444';
                                ...
                                suggestion = `Feature as a Happy Hour Special between 2–4 PM!`;
                            ...
                            return { name, qty: data.qty, dropPct, suggestion, discountLabel, promoColor, promoIcon, urgency, urgencyColor };
                        });
                    ...
                    return (
                        <div style={{
                                ...
                                <div style={{ display: 'flex', alignItems: 'center', gap: '18px' }}>
                                    ...
                                    <div>
                                        <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
                                            <h3 style={{ margin: 0, fontSize: '1.35rem', fontWeight: 900, color: 'white', letterSpacing: '-0.3px' }}>
                                                Low Demand Auto Promotion
                                            </h3>
                                            <div style={{
                                                background: 'linear-gradient(135deg, #f59e0b, #ef4444)',
                                                borderRadius: '20px',
                                        ...
                                        <p style={{ margin: '4px 0 0', color: 'rgba(255,255,255,0.55)', fontSize: '0.85rem', fontWeight: 500 }}>
                                            {isAllGood
                                                ? 'All items are performing well — no promotions needed!'
                                                : `System detected ${lowDemandItems.length} under-performing item${lowDemandItems.length > 1 ? 's' : ''} — promotions auto-suggested`
                                            }
                                        </p>
                                    </div>
                                </div>
                                {!isAllGood && (
                                    <div style={{
                                        ...
                                        <span style={{ color: '#f59e0b', fontSize: '0.8rem', fontWeight: 700 }}>
                                            {lowDemandItems.filter(i => i.urgency === 'HIGH').length} HIGH urgency
                                        </span>
                                    </div>
                                )}
                            </div>
                            ...
                            {/* Panel Body */}
                            <div style={{ background: '#f8fafc', padding: '30px 36px' }}>
                                {isAllGood ? (
                                    <div style={{
                                        display: 'flex', flexDirection: 'column', alignItems: 'center',
                                        justifyContent: 'center', gap: '14px', padding: '40px 20px'
                                    }}>
                                        <div style={{
                                        ...
                                        <p style={{ fontWeight: 800, fontSize: '1.15rem', color: '#1e293b', margin: 0 }}>All items are performing well 🎉</p>
                                        <p style={{ color: '#94a3b8', fontSize: '0.85rem', margin: 0 }}>No promotions needed right now. Keep up the great work!</p>
                                    </div>
                                ) : (
                                        ...
                                        <AnimatePresence>
                                            {lowDemandItems.map((item) => (
                                                <motion.div
                                                    ...
                                                    style={{
                                                        background: appliedPromotions.has(item.name)
                                                            ? 'linear-gradient(135deg, #ecfdf5, #d1fae5)'
                                                            : 'white',
                                                        borderRadius: '20px',
                                                        padding: '24px',
                                                        border: appliedPromotions.has(item.name)
                                                            ? '1.5px solid #10b981'
                                                            : '1.5px solid #f1f5f9',
                                                ...
                                                >
                                                        ...
                                                        <div style={{ flex: 1, minWidth: 0 }}>
                                                            ...
                                                            <div style={{ display: 'flex', alignItems: 'center', gap: '6px', marginTop: '6px' }}>
                                                                ...
                                                                <span style={{ fontSize: '0.78rem', color: '#64748b', fontWeight: 600 }}>
                                                                    {item.qty} sold · {item.dropPct}% below avg
                                                                </span>
                                                            </div>
                                                        </div>
                                                        <div style={{
                                                            background: item.promoColor + '15',
                                                            border: `1px solid ${item.promoColor}30`,
                                                            borderRadius: '10px',
                                                            padding: '6px 10px',
                                                            display: 'flex', alignItems: 'center', gap: '5px',
                                                            color: item.promoColor,
                                                            fontSize: '0.7rem',
                                                            fontWeight: 800,
                                                            whiteSpace: 'nowrap',
                                                            flexShrink: 0
                                                    ...
                                                    {/* Suggestion Box */}
                                                    <div style={{
                                                        background: `linear-gradient(135deg, ${item.promoColor}08, ${item.promoColor}04)`,
                                                        border: `1px dashed ${item.promoColor}30`,
                                                        borderRadius: '14px',
                                                        padding: '14px 16px',
                                                        display: 'flex', alignItems: 'flex-start', gap: '10px'
                                                    }}>
                                                        <Sparkles size={15} color={item.promoColor} style={{ flexShrink: 0, marginTop: '1px' }} />
                                                        <span style={{ fontSize: '0.82rem', color: '#334155', fontWeight: 600, lineHeight: 1.5 }}>
                                                            {item.suggestion}
                                                        </span>
                                                    </div>
                                                    ...
                                                    {appliedPromotions.has(item.name) ? (
                                                        <div style={{
                                                            ...
                                                            background: '#d1fae5', borderRadius: '14px',
                                                        ...
                                                        <div style={{ display: 'flex', gap: '10px' }}>
                                                            <button
                                                                ...
                                                                style={{
                                                                    ...
                                                                    borderRadius: '14px',
                                                                    background: `linear-gradient(135deg, ${item.promoColor}, ${item.promoColor}cc)`,
                                                                    color: 'white', border: 'none',
                                                                    fontWeight: 800, fontSize: '0.82rem',
                                                                    cursor: 'pointer',
                                                                    display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '6px',
                                                                    boxShadow: `0 4px 14px ${item.promoColor}40`,
                                                                    transition: 'all 0.2s ease'
                                                                }}
